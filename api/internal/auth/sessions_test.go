...
func TestSessions_Lookup_Expired(t *testing.T) {
	...
	if _, err := s.DB.ExecContext(t.Context(), `UPDATE sessions SET expires_at = ? WHERE token = ?`,
...
func TestSessions_Lookup_CorruptExpires(t *testing.T) {
	...
	if _, err := s.DB.ExecContext(t.Context(), `UPDATE sessions SET expires_at = 'garbage' WHERE token = ?`, tok); err != nil {
		t.Fatalf("update: %v", err)
	}
	if _, _, err := store.lookup(context.Background(), tok); err == nil ||
	...
	var n int
	_ = s.DB.QueryRowContext(t.Context(), `SELECT COUNT(*) FROM sessions WHERE token = ?`, tok).Scan(&n)
	if n != 0 {
...
func TestSessions_Authenticate(t *testing.T) {
	...
	t.Run("mutating without csrf returns 403", func(t *testing.T) {
		rr := httptest.NewRecorder()
		req := httptest.NewRequestWithContext(t.Context(), "POST", "/protected", nil)
		req.AddCookie(&http.Cookie{Name: sessionCookie, Value: tok})
		h.ServeHTTP(rr, req)
...
func TestSessions_DeleteForUser(t *testing.T) {
	...
	var n int
	_ = s.DB.QueryRowContext(t.Context(), `SELECT COUNT(*) FROM sessions WHERE user_id=1`).Scan(&n)
	if n != 0 {
...
func TestSessions_HandleLogout(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	req := httptest.NewRequestWithContext(t.Context(), "POST", "/logout", nil)
	req.AddCookie(&http.Cookie{Name: sessionCookie, Value: tok})
...
func TestSessions_GCOnce_DeletesExpired(t *testing.T) {
	...
	store := NewSessionStore(s)
	tok, _, _ := store.Create(context.Background(), 1)
	_, _ = s.DB.ExecContext(t.Context(), `UPDATE sessions SET expires_at = ? WHERE token = ?`,
		time.Now().Add(-time.Hour).UTC().Format(time.RFC3339), tok)
	store.gcOnce(context.Background())
	var n int
	_ = s.DB.QueryRowContext(t.Context(), `SELECT COUNT(*) FROM sessions`).Scan(&n)
	if n != 0 {
