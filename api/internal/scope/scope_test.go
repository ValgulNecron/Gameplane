...
func TestResolve(t *testing.T) {
	...
	t.Run("default explicitly allowed", func(t *testing.T) {
		req := httptest.NewRequestWithContext(t.Context(), "GET", "/?namespace="+DefaultNamespace, nil)
		got, err := Resolve(req)
		if err != nil || got != DefaultNamespace {
			t.Fatalf("got %q err=%v", got, err)
		}
	...
	t.Run("allow-listed extra namespace resolves", func(t *testing.T) {
		req := httptest.NewRequestWithContext(t.Context(), "GET", "/?namespace=extra", nil)
		got, err := Resolve(req)
	...
	t.Run("unknown namespace forbidden", func(t *testing.T) {
		req := httptest.NewRequestWithContext(t.Context(), "GET", "/?namespace=other", nil)
		_, err := Resolve(req)
