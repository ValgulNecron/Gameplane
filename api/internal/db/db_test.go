...
func TestOpen_SQLiteAndMigrate(t *testing.T) {
	...
	// Schema is in place.
	var n int
	if err := s.DB.QueryRowContext(context.Background(), `SELECT COUNT(*) FROM schema_migrations`).Scan(&n); err != nil {
...
func TestMigrate_SeedsBuiltinRoles(t *testing.T) {
	...
	for _, name := range []string{"admin", "operator", "viewer"} {
		var builtin int
		if err := s.DB.QueryRowContext(context.Background(), `SELECT builtin FROM roles WHERE name = ?`, name).Scan(&builtin); err != nil {
...
func TestAdoptLegacySQLite_TargetAbsentLegacyPresent(t *testing.T) {
	...
	if _, err := legacyDB.ExecContext(context.Background(), `CREATE TABLE test_data (id INTEGER, name TEXT)`); err != nil {
		t.Fatalf("create table: %v", err)
	}
	if _, err := legacyDB.ExecContext(context.Background(), `INSERT INTO test_data VALUES (1, 'alice')`); err != nil {
	...
	var id int
	var name string
	if err := targetDB.QueryRowContext(context.Background(), `SELECT id, name FROM test_data WHERE id = 1`).Scan(&id, &name); err != nil {
...
func TestAdoptLegacySQLite_Sidecars(t *testing.T) {
	...
	if _, err := legacyDB.ExecContext(context.Background(), `CREATE TABLE t (id INTEGER)`); err != nil {
		t.Fatalf("create table: %v", err)
	}
	legacyDB.Close()
...
func TestAdoptLegacySQLite_TargetExists(t *testing.T) {
	...
	if _, err := legacyDB.ExecContext(context.Background(), `CREATE TABLE legacy_data (value TEXT)`); err != nil {
		t.Fatalf("create legacy table: %v", err)
	}
	if _, err := legacyDB.ExecContext(context.Background(), `INSERT INTO legacy_data VALUES ('from_legacy')`); err != nil {
	...
	if _, err := targetDB.ExecContext(context.Background(), `CREATE TABLE target_data (value TEXT)`); err != nil {
		t.Fatalf("create target table: %v", err)
	}
	if _, err := targetDB.ExecContext(context.Background(), `INSERT INTO target_data VALUES ('from_target')`); err != nil {
	...
	defer targetDB.Close()
	var val string
	if err := targetDB.QueryRowContext(context.Background(), `SELECT value FROM target_data WHERE value = 'from_target'`).Scan(&val); err != nil {
...
func TestMigrate_AddsAuditChainColumns(t *testing.T) {
	...
	if err := s.DB.QueryRowContext(context.Background(), `SELECT prev_hash, hash FROM audit_events LIMIT 1`).Scan(&prevHash, &hash); err != nil {
		t.Fatalf("query: %v", err)
	}
...
func TestOpen_AdoptsLegacySQLite(t *testing.T) {
	...
	if _, err := legacyDB.ExecContext(context.Background(), `CREATE TABLE IF NOT EXISTS schema_migrations (version TEXT PRIMARY KEY, applied_at TEXT NOT NULL)`); err != nil {
		t.Fatalf("create schema_migrations: %v", err)
	}
	if _, err := legacyDB.ExecContext(context.Background(), `INSERT INTO schema_migrations VALUES ('001_init.sql', datetime('now'))`); err != nil {
		t.Fatalf("seed migration: %v", err)
	}
	if _, err := legacyDB.ExecContext(context.Background(), `CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE, role TEXT NOT NULL DEFAULT 'viewer')`); err != nil {
		t.Fatalf("create users: %v", err)
	}
	if _, err := legacyDB.ExecContext(context.Background(), `INSERT INTO users(id, username, role) VALUES (1, 'Alice', 'admin')`); err != nil {
		t.Fatalf("insert user: %v", err)
	}
	if _, err := legacyDB.ExecContext(context.Background(), `CREATE TABLE audit_events (
	...
	if _, err := legacyDB.ExecContext(context.Background(), `CREATE INDEX idx_audit_ts ON audit_events(ts DESC)`); err != nil {
	...
	var username string
	if err := store.DB.QueryRowContext(context.Background(), `SELECT username FROM users WHERE id = 1`).Scan(&username); err != nil {
