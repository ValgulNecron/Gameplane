...
Note: Helm's own `crds/` install runs only on first install and `helm upgrade` never updates it — so the chart ships a **pre-install/pre-upgrade hook** (`crds.autoApply`, in `charts/gameplane/templates/crd-apply-hook.yaml`) that runs `kubectl apply --server-side` over the CRDs on every install AND upgrade. This works for any `helm upgrade` in any environment, not just `make dev-install`. The CRDs it applies are shipped in `charts/gameplane/crd-manifests/` (a `.Files`-readable copy of `crds/`, since Helm hides the special `crds/` dir from `.Files`); `make manifests` keeps both copies in sync. CRDs are still never owned/deleted by Helm, so `helm uninstall` leaves GameServers intact.
