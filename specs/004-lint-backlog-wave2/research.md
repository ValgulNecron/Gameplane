...
| Can new suppression directives be introduced? | No; Constitution III forbids silencing, and the feature preserves zero-suppression property. | **Decision 5** below; `.golangci.yml` started this work with exactly three config-level exemptions (_test.go, internal/controller revive, gameproto minecraft.go G115) and gained five more, narrowly-scoped and reviewed (eight total, inventoried in `contracts/exclusion-policy.md`); zero in-source directives exist anywhere, and none were added. |
...
- `.golangci.yml`: eight config-level exclusion rules total, inventoried in `contracts/exclusion-policy.md`. No in-source directives, no per-module configs.
