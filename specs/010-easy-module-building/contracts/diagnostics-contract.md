...
| `image-unpinned` | **ERROR** | `template.yaml` | Default `spec.image` does not include an `@sha256:...` digest pin and lacks the `# gameplane:floating` comment opt-out. | Pin image digest using `gp-module pin` / `make module-pin` or append `# gameplane:floating` if intentionally dynamic. |
