...
- Dark and light appearance: the current dashboard defaults to dark. The rebuild must render correctly in both appearances with the Gameplane brand tokens defined in `contracts/theme-tokens.md` (pink accent, superseding the original orange — Settled 2026-09-04, maintainer, in-session) applied to the HeroUI palette.
...
- **FR-006**: The rebuilt interface MUST apply the Gameplane brand tokens (pink accent per `contracts/theme-tokens.md`, superseding the original orange per OD-8 — Settled 2026-09-04, existing success/warning/danger/violet scale, Geist and JetBrains Mono type) onto the HeroUI theme in both dark and light appearance, dark remaining the default.
...
- **FR-015**: The dashboard MUST apply the Gameplane brand palette defined in `contracts/theme-tokens.md` in both dark and light appearance, with no screen referencing the lunaris library colour variables (`$c:…` in Pencil or the legacy `--color-*` hex aliases in code) after it is rebuilt.
...
- Q: Keep the Gameplane brand tokens on HeroUI with dark and light, or adopt HeroUI's default theme? → A: Gameplane tokens (orange primary, current scale) mapped onto HeroUI; dark stays the default and light is supported (FR-006). Superseded 2026-09-04: the maintainer replaced the orange primary with a pink accent brand re-theme after reviewing the slice-0 atoms; see OD-8 and `contracts/theme-tokens.md`.
