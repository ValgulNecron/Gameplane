...
- Nuclear Option's remote-command JSON protocol is documented in third-party sources, not the official Steam documentation. If the protocol changes in a future game update, how is drift detected and handled? (The answer is real-world protocol-level E2E testing, per the constitution.)
- The Nuclear Option dedicated server is believed to be a separate binary (app ID 3930080, per third-party hosting-provider documentation, not confirmed against Steam's own store listing) from the base game (app 2168680). Can the operator update the binary without manually pulling a new module/image? (This is a templating/upgrade-policy question, out of scope for this spec but noted as a known gap.)
- Nuclear Option's server logs are assumed to be readable by the agent for status/backup purposes, consistent with every other supported module. The exact on-disk log location for this specific game has not been confirmed against a live server and must be verified during implementation.
- Does the dedicated server expose any signal that distinguishes "process started" from "actually accepting player connections"? This has not been confirmed. If no such signal exists, "Accepting Players" status (User Story 1, Acceptance Scenario 2) must be inferred conservatively (e.g., from the same real-protocol probe used by the join test) rather than assumed to exist.
...
- **FR-004**: The Nuclear Option server MUST be fully joinable by real game clients using the game's actual wire protocol (game port assumed to be UDP 7777 per third-party hosting-provider documentation; see Assumptions for the confirmation caveat) — a real join test, not a mock handshake or bare socket dial, MUST exist in the E2E suite per the project constitution.
- **FR-005**: The Nuclear Option join test MUST be proven to fail when run against a non-listening address and to succeed when run against a real, booted Nuclear Option server before it is trusted as a correctness signal for the module, per the project constitution's E2E-Tested Delivery principle.
- **FR-006**: The Nuclear Option server's Remote Console interface MUST support the remote-command protocol as currently documented in available third-party sources (assumed to be a length-prefixed JSON request/response protocol on a dedicated remote-command port, separate from the game and query ports). Because this protocol is not officially documented by the game's publisher, its exact shape (port, framing, command names, result codes) MUST be confirmed against a real running server as part of implementation, and the E2E join test's Remote Console coverage MUST catch drift if the assumed protocol proves incorrect. See Assumptions for the full caveat.
...
- **FR-026**: E2E test coverage for Nuclear Option MUST include a real protocol-level join test. If the game server is too resource-heavy to run in the default CI runners, the test MUST still be authored and committed to `test/e2e/`; it MUST be excluded from all CI bucket execution in `test/e2e/buckets.sh` with a comment explaining the exclusion, and MUST remain runnable on demand (against a manually provided cluster or triggered via workflow).
...
- **SC-005**: 100% of shipped game modules (including Nuclear Option) have a committed E2E join test covering the real protocol, whether executed in CI or reserved as deferred-on-demand; zero modules have unknown or missing coverage.
- **SC-006**: Servers created without a pool preference set (backward-compatible case) continue to receive addresses from the default pool with no change in behavior or latency compared to the previous release.
...
- The Nuclear Option dedicated server (believed to be Steam app 3930080, per third-party hosting-provider documentation, not the publisher's own listing) is assumed to be publicly downloadable without requiring ownership of the base game, using the Linux native build (no Windows compatibility layer). **None of this is confirmed against the publisher.** If it proves false (e.g., licensing restrictions on the dedicated server, no Linux build, base-game ownership required), the feature's scope will shift to document the licensing/platform blocker rather than proceeding with an unlicensable or unbuildable template.
- The exact network footprint of the dedicated server — a UDP game port, a separate UDP query port, and a separate remote-command port (commonly cited as 7777/7778/7779 in third-party hosting-provider docs) — is an assumption, not a confirmed fact. The remote-command protocol itself (assumed to be a length-prefixed JSON request/response scheme with numeric result-code ranges for success/client-error/server-error) is documented only in third-party operator guides and forums, never by the publisher. All of these details — the exact ports, the wire format, the specific command names, and the result-code semantics — MUST be verified against a real running server during implementation, not taken as given. The real-protocol E2E join test (FR-004, FR-005) is the mechanism that catches drift if any of these assumptions is wrong or changes upstream; the moderation commands (FR-007–FR-012) may require rework in response.
- Whether the dedicated server exposes any signal distinguishing "process has started" from "server is actually accepting player connections" is unverified (see Edge Cases). Readiness reporting in the dashboard must be derived from the most reliable signal actually available once this is confirmed, not assumed to be a simple process-liveness check.
