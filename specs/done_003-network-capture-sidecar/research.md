...
**Amendment (2026-08-24)**: The manifest DOES set `capabilities: {drop: ["ALL"], add: ["NET_RAW"]}` — this does not reverse the decision above. `drop: ["ALL"]` alone empties the process's *bounding* capability set, not just its effective set, and the kernel refuses to grant a file capability at `execve` for a capability that isn't in the bounding set (the socket open would fail with `EPERM`). `add: ["NET_RAW"]` is present solely to keep NET_RAW in the bounding set so the setcap'd binary's file capability can still be granted; the process's effective capability set is still empty at container start, and the grant itself is still the file capability, not this `add`. Every passage below discussing "does not rely on capabilities.add" should be read as "does not rely on capabilities.add *alone* to grant the capability."
...
**EVIDENCE**: api/internal/rbac/catalog.go:85-89 (ValidPermission rejects "*" only), api/internal/rbac/catalog.go:3-5 ("* is never grantable through the API" comment), api/internal/rbac/rbac_test.go:68-82 (test explicitly states "*" is "the sole intentional exception"), api/internal/rbac/rbac.go:179 (admin catch-all uses perm:"*", not a special marker), api/internal/db/migrations/003_roles.sql:43-70 (permissions seeded to roles without structural restrictions beyond wildcard), specs/done_003-network-capture-sidecar/tasks.md:T012 (Decision 6 explicitly flagged as unresolved with two options)
