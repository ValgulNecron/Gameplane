...
- **Capability isolation (human Decision 4)**: `CAP_NET_RAW` on the capture container only, granted via file capabilities on its binary (`setcap cap_net_raw+ep`), not via a `securityContext.capabilities.add` grant alone — Kubernetes does not set ambient capabilities, so `add: ["NET_RAW"]` under a non-root `runAsUser` would grant nothing (D-SETCAP). This requires `allowPrivilegeEscalation: true` on the capture container's `securityContext` (file capabilities are ignored under `no_new_privs`); `runAsNonRoot` stays true, `allowPrivilegeEscalation` is the accepted trade. The game container's securityContext is never touched.
...
| **Elevated Linux capability (`CAP_NET_RAW`) on the capture container, granted via file capabilities (D-SETCAP, human Decision 4, final; amended 2026-08-24 — see research.md)** | AF_PACKET raw-socket capture is not possible without it. The grant mechanism is **file capabilities on the capture binary** (`setcap cap_net_raw+ep`), not `securityContext.capabilities.add` alone — Kubernetes does not set ambient capabilities, so `add: ["NET_RAW"]` by itself under a non-root `runAsUser` grants nothing (the effective set is cleared on `execve`). The manifest lists `add: ["NET_RAW"]` alongside `drop: ["ALL"]` anyway, solely to keep NET_RAW in the process's *bounding* set — `drop: ["ALL"]` alone would empty the bounding set too, and the kernel refuses to grant the setcap'd binary's file capability at `execve` when the capability isn't in the bounding set. Isolated to the capture container's own binary; the game container's security posture is left untouched. | An unprivileged capture mechanism (eBPF-based) was considered and deferred (research.md, Security Posture section) as newer and less proven across the range of game workloads this project targets. Running the capture container as root instead of using file capabilities was rejected as a strictly worse trade — it gives up `runAsNonRoot` entirely rather than the narrower `allowPrivilegeEscalation` concession below. This does not eliminate risk — a cluster enforcing `PodSecurity: restricted` on the games namespace will reject these pods (see the next two rows), which is recorded as a constraint, not resolved by this feature. |
