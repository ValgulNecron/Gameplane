...
Confirms that no in-source linting suppression directives exist in the tree; only the nine authorized config-level exclusions in `.golangci.yml` (inventoried in `contracts/exclusion-policy.md`), including the pre-existing gosec G115 exclusion in the Minecraft codec and the global G104 exclusion, are permitted. This is a static, local check.
...
# One of the nine authorized config-level exclusions (Minecraft VarInt codec);
...
| **SC-002**: Zero in-source suppression directives exist; only the nine authorized config-level `.golangci.yml` exclusions are present (contracts/exclusion-policy.md) | Scenario 2 | Static/Local | Grep over tree for //nolint, #nosec, lint:ignore returns no matches; all nine exclusions (8 path-scoped + 1 global G104) are in .golangci.yml config, not inline |
