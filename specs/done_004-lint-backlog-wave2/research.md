...
| Can new suppression directives be introduced? | No; Constitution III forbids silencing, and the feature preserves zero-suppression property. | **Decision 5** below; `.golangci.yml` started this work with exactly three config-level exemptions (_test.go, internal/controller revive, gameproto minecraft.go G115) and gained five more, narrowly-scoped and reviewed (nine total—eight path-scoped rules plus one global gosec setting, inventoried in `contracts/exclusion-policy.md`); zero in-source directives exist anywhere, and none were added. |
...
**Statement**: The `.golangci.yml` configuration started wave 2 with exactly three config-level exemptions: (1) `_test.go` files are exempt from errcheck, gosec, and unparam (because test setup often ignores errors deliberately); (2) `internal/controller/` is exempt from revive's `exported:` rule (because reconciler builders have a lot of repeated patterns); (3) `gameproto/minecraft.go` is exempt from gosec's G115 (because VarInt encoding requires lossless reinterpretation between uint32 and int32, with explicit overflow checks in code). Wave 2 work reviewed and authorized five more, narrowly-scoped config-level exclusions (nine total—eight path-scoped rules plus one global gosec setting, inventoried in `contracts/exclusion-policy.md`). No new `//nolint`, `// nosec`, `// lint:ignore`, or equivalent in-source suppression directives were introduced during wave 2 work. The zero-suppression property — no inline directives in source code — is preserved absolutely. Every finding is fixed with real code changes.
...
Wave 1 cleared 582 findings across 8 modules and added exactly one new config-level exclusion (the G115 exclusion in gameproto's minecraft.go). This is the authoritative precedent: when a finding is real but unavoidable (like VarInt's two's complement reinterpretation), the exclusion is documented with a comment (lines 47-52 in .golangci.yml) explaining why the finding is safe and what explicit checking is in place. Never an in-source directive.

Wave 2 followed the same discipline: five genuine false positives were raised with the maintainer and reviewed with the same rigor as the Minecraft G115 case, each landing as its own narrow, documented config-level exclusion — never silenced inline. `.golangci.yml` now carries nine such exclusions total—eight path-scoped rules plus one global gosec setting (see `contracts/exclusion-policy.md`); zero in-source suppression directives exist anywhere in the tree.
...
- `.golangci.yml`: nine config-level exclusion rules total—eight path-scoped rules plus one global gosec setting, inventoried in `contracts/exclusion-policy.md`. No in-source directives, no per-module configs.
...
- Spec FR-002 (spec.md line 165-169): "Every linting finding reported by golangci-lint in `api`, `agent`, and `test/e2e` MUST be resolved via code changes... Suppression directives (`//nolint`, `//#nosec`, etc.) MUST NOT be introduced anywhere in the tree — this remains absolute."
- Spec FR-005 (spec.md line 181-188): "The feature MUST preserve the existing zero-inline-suppression property of the tree: no new in-source suppression directives... are introduced at any point during Wave 2 work, and none exist anywhere in the tree at completion. This is distinct from config-level exclusions in `.golangci.yml`, which are narrow, per-file, documented, and maintainer-authorized; several such exclusions were authorized during Wave 2 work (see `contracts/exclusion-policy.md`), and adding them does not weaken the inline-suppression prohibition."
