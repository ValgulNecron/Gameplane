...
- [X] T049 CRITICAL: Add the three missing OIDC login-time e2e tests to test/e2e/api_auth_e2e_test.go via a fake OIDC IdP fixture — (a) a user in a Helm-seeded admin group receives the admin role on first login, (b) the role is re-evaluated and updated on a subsequent login after the group changes, (c) a user matching no mapping receives the default role — and register all three in the api-auth bucket in test/e2e/buckets.sh within the ~7-login-per-job budget, per T033/T034 and Constitution Principle I
...
**Purpose**: Close gaps found by a second `/speckit-converge` pass between the artifacts (spec.md, plan.md, tasks.md) and the state of the code. Appended without modifying any existing task.

- [X] T053 Add the two missing OIDC login-time e2e tests that T033 specified but were never written — (a) an admin `PUT /admin/config/auth` sets a `helmOverride.roleMappings` list for a role that differs from the Helm-seeded mapping, and a subsequent `oidcHelmLogin` resolves the override value with no API restart; (b) the admin then calls `DELETE /admin/config/auth/role-mappings/{role}` and a subsequent `oidcHelmLogin` resolves back to the Helm-seeded value — in test/e2e/api_auth_e2e_test.go, per SC-007 / T033 items 4-5 / plan.md Slice 10 (missing). **Bucket conflict to resolve**: T034 says register in `api-auth`, but test/e2e/buckets.sh:113-124 explicitly warns that a test writing `helmOverride.roleMappings` must not share `api-auth` with the `TestAPI_OIDCHelmSeeded_*` tests (it would flake them); place these two in `api-roles` alongside `TestAPI_AuthConfig_RoleMappings` (or a new bucket) with a comment recording the rationale, and keep within the ~7-login-per-job budget
- [X] T054 Add a lower-tier regression test proving the override is read live per login rather than snapshotted at startup: wire `AttachHelmRoleOverridesFunc` to a mutable store and drive the OIDC callback twice with a config mutation in between, asserting the resolved role changes without restart — in api/internal/auth/oidc_rolemap_test.go, per SC-007 (missing). No test in the tree currently exercises `AttachHelmRoleOverridesFunc` at all; `effectiveHelmPolicy` is only unit-tested against hand-built inputs
- [X] T055 Correct the contradictory note at docs/oidc.md:21 — it states that group->role mapping "is only available on dashboard-registered providers" and that the Helm-flag OIDC provider "does not support it", which contradicts the same file's "Role Mapping at Install Time" section (docs/oidc.md:105-179), the Okta example (docs/oidc.md:157-158), docs/install.md:130-147, and the shipped flags in api/cmd/main.go:453-461,151-164 — per FR-016 (contradicts)
- [X] T056 Reject rather than silently drop empty entries in the comma-separated `--oidc-role-mapping-{admin,operator,viewer}` values: `parseRoleMapping` in api/cmd/main.go:569-582 trims and discards empty tokens, while T006 requires rejection (matching the hard-fail `--oidc-default-role` validation at api/cmd/main.go:77-89); add the validation and a unit test in api/cmd/main_test.go covering the accept and reject paths, per T006 (partial)
- [X] T057 Fix or remove the stale doc comment at api/internal/handlers/config_envtest_test.go:310-311, which defers login-merge coverage to `auth_envtest_test.go` — a file that does not exist anywhere in the repo — and point it at the real coverage added by T053/T054 instead, per Constitution Principle IV (partial)
