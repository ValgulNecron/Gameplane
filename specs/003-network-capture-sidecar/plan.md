...
- **Capture files**: emptyDir volume at `/tmp/captures/capture-<capture-id>.pcapng` (ephemeral, lost on pod restart), pre-provisioned on every game pod's StatefulSet template regardless of opt-in (human Decision 1), ideally with a `sizeLimit`.
- **Audit trail**: existing `audit_events` SQLite/pgx table, plus a new append-only migration adding a structured `reason` column (human Decision 2) — see contracts/audit-events.md.
...
- **Test coverage**: one structural E2E test in the operator bucket (PLANNED, not written).
...
- **Justification**: The spec-kit lifecycle has genuinely been followed through this stage — spec.md, research.md, data-model.md, quickstart.md, and contracts/ all exist and were read in full for this revision. They are substantially more consistent than the prior draft on the RBAC mechanism, audit schema, ephemeral-container behavior, and status shape, and four architectural decisions (capture storage pre-provisioning, audit reason column, error response format, 24-hour retention default) are now settled by explicit human decision rather than left as competing proposals. They are **not**, however, fully internally consistent as a set: this same document's "Phase 0 & 1 Artifacts" section below records a still-open cross-artifact inconsistency (migration numbering and the audit route shape used by `contracts/rest-api.md` vs. `contracts/audit-events.md`) that this revision does not resolve. What remains PLANNED: Constitution Principle IV also requires every module folder to maintain a `specs.md`; the new `capture-sidecar/` module does not exist yet and therefore has no `specs.md` yet. This must be created in the same change that adds the module's code (Phase 2), not deferred past it — tracked in Complexity Tracking (retained from the prior draft, assessed as a solid entry).
...
│   └── audit-events.md   # Audit event schema against the real `audit.Event` struct; a structured `reason` column and its hash-chain interaction are RATIFIED (human Decision 2)
└── tasks.md              # Phase 2 output (implementation task breakdown — to be created by /speckit-tasks; does not exist yet)
```
...
│                                            #   for every game pod (human Decision 1) — today's pod template
...
│   ├── db/migrations/
│   │   ├── 007_captures_rbac.sql      # NEW, append-only: grants captures:manage to admin only
│   │   └── 007_audit_reason.sql       # NEW, append-only (human Decision 2 — RATIFIED): adds a nullable,
│   │                                  #   structured audit_events.reason column, next after the six files
│   │                                  #   (001-006) that currently exist in this directory
│   │                                  #   — NOTE: rest-api.md and audit-events.md each independently
│   │                                  #   propose "007_" as the next migration number; the two new
│   │                                  #   migrations cannot both be 007, and this plan does not resolve
│   │                                  #   which one is renumbered — that is Phase 2 implementer work.
│   └── audit/
│       └── audit.go                   # MODIFIED (EXISTING file), human Decision 2 (RATIFIED, mechanism
...
**E2E tests**: `test/e2e/buckets.sh` and `test/e2e/gameserver_e2e_test.go` EXIST

```text
test/e2e/
├── buckets.sh                 # MODIFIED (EXISTING file): add TestGameServer_NetworkCaptureEphemeralContainer
│                               #   to the operator bucket
└── gameserver_e2e_test.go     # MODIFIED (EXISTING file): add the structural capture test function (PLANNED,
                                #   not written)
```
...
7. **contracts/audit-events.md**: audit event schema reconciled against the real
   ...
   hash-chain interaction (human Decision 2, RATIFIED) as this feature's canonical audit
   design, including how a pre-migration row must keep verifying against its original,
   pre-`reason` hash.

**Known unresolved inconsistency across the Phase 1 artifacts** (flagged, not fixed by
this plan, since this revision touches only `plan.md`): `contracts/rest-api.md` and
`contracts/audit-events.md` independently propose the next migration file be numbered
`007_` — `007_captures_rbac.sql` and `007_audit_reason.sql` cannot both hold that number
against the six files (`001`–`006`) that exist today. This numbering collision must be
resolved — one of the two renumbered — before Phase 2 implementation of either migration
begins. The earlier route-shape mismatch between the two contracts (`PATCH
/servers/{name}` vs. `:capture-enable`/`:capture-disable`) has since been reconciled in
`contracts/audit-events.md` and is no longer open.

**Next step**: generate `tasks.md` via `/speckit-tasks`, resolving the cross-artifact
inconsistency noted above as part of task breakdown, before any implementation begins.
