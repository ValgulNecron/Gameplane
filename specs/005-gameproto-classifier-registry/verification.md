...
| SC-007 | Stream replay (Consumed bytes) preserved | SATISFIED | **Minecraft**: `gameproto/classifier_golden_test.go:129-134` contains a real assertion (`bytes.Equal` with `t.Errorf`) verifying that `Consumed` bytes match input for successful parses. **Terraria**: `gameproto/classifier_golden_test.go:390-392` is a non-asserting `t.Logf` (style inconsistency only); Terraria's Consumed-byte equivalence IS properly asserted by dedicated unit tests: `gameproto/terraria_test.go:234-235` (`TestClassifyTerrariaConsumedBytes` asserts `bytes.Equal(result.Consumed, data)`), and `gameproto/terraria_test.go:344-345` (`TestTerrariaConsumedBytesWithPipelinedData` asserts `bytes.Equal(result.Consumed, connect)` — proving Consumed covers handshake only, excluding trailing pipelined bytes). Stream replay is further verified by sentinel e2e integration (`sentinel/main.go:511-539` replays Consumed bytes; `TestGameServer_WakeOnConnect_LoginWakes` passes). (SUPERSEDED: the equivalence test file was deleted per PR #248 and replaced by the golden suite; Consumed-byte coverage is as scoped above.) |
