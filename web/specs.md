...
10. **Capture** *(PLANNED — feature 003, not yet implemented; see `specs/003-network-capture-sidecar/`)* — a `CaptureWidget` component driving start/stop of packet captures and a table of past captures for this server, gated on the `captures:manage` permission. Sits between the Backups and Settings tabs per `design-export/json` node `O08uaD`/`b4eaUf` (start-capture modal) and `m5kOm4` (capture list). None of `CaptureWidget.tsx`, its `lib/endpoints.ts` `Captures` namespace, or its `router`/tab wiring exist in `web/src` as of this writing — do not assume line numbers or exact file names for them without re-reading the tree.
...
**Capture types (`src/types.ts`, built):** `CaptureConfiguration` (`{ enabled?, retentionSeconds? }`, mirrors `spec.capture` — note as of this writing `types.ts`'s comment on `retentionSeconds` says "60..7776000", which does not match the CRD's authoritative `+kubebuilder:validation:Minimum=1 / Maximum=604800` on `operator/api/v1alpha1/gameserver_types.go`'s `CaptureConfiguration.RetentionSeconds` — re-check before relying on that comment), `CaptureStatus` (mirrors `status.capture`: `ready`, `activeCapture`/`lastCaptureTime` typed nullable since the API's `formatOptionalTime` never omits the key), `CapturePhase` (`"Pending" | "Running" | "Completed" | "Failed" | "Expired"`), `NetworkCapture` (one capture record — merges the API's start/stop/list/get response shapes) and `NetworkCaptureList` (the `:captures` list envelope). These type mirrors are already in the tree; the `CaptureWidget` component and `Captures` endpoint namespace that would consume them are PLANNED (see above).
