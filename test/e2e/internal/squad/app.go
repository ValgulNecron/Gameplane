// Package main implements a join-depth probe for Squad dedicated servers.
package main
...
func main() {
	...
	depth, evidence, err := probeSquad(ctx, *addr)
...
// probeSquad executes join-depth probing against the target game server.
func probeSquad(ctx context.Context, addr string) (joindepth.JoinDepth, string, error) {
	var info *a2s.Info
	err := retryWithContext(ctx, "squad-a2s-query", 15*time.Second, func(actx context.Context) error {
		var err error
	...
	if err != nil {
		return joindepth.JoinDepth(-1), "", fmt.Errorf("squad a2s query failed: %w", err)
	}
