...
const (
	...
	// NeedsAuth is the outcome when the server sent an Encryption Request, i.e. it runs in
	// online-mode and requires a real (Mojang-authenticated) account.
	NeedsAuth
	// Disconnected is the outcome when the server refused the login (Disconnect packet).
	Disconnected
)
...
func Login(ctx context.Context, addr string, protocol int, username string) (*LoginResult, error) {
	...
	// instead.
	if len(username) > 16 {
		return nil, fmt.Errorf("minecraft: username %q is %d characters; Minecraft allows at most 16",
	...
	for {
		id, payload, err := rd.readPacket()
		if err != nil {
			return nil, fmt.Errorf("minecraft: read login response: %w", err)
		}
		switch id {
