// Package main implements a join-depth probe for Left 4 Dead 2 dedicated servers.
package main
...
func main() {
	...
	depth, evidence, err := probeL4D2(ctx, *addr)
...
// probeL4D2 executes join-depth probing against the target game server.
func probeL4D2(ctx context.Context, addr string) (joindepth.JoinDepth, string, error) {
	var info *a2s.Info
	err := retryWithContext(ctx, "l4d2-a2s-query", 15*time.Second, func(actx context.Context) error {
		var err error
	...
	if err != nil {
		return joindepth.JoinDepth(-1), "", fmt.Errorf("l4d2 a2s query failed: %w", err)
	}
	...
	_ = retryWithContext(ctx, "l4d2-source-challenge", 5*time.Second, func(actx context.Context) error {
