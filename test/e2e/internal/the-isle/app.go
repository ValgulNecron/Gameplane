// Package main implements a join-depth probe for The Isle dedicated servers.
package main
...
func main() {
	...
	depth, evidence, err := probeTheIsle(ctx, *addr)
...
// probeTheIsle executes join-depth probing against the target game server.
func probeTheIsle(ctx context.Context, addr string) (joindepth.JoinDepth, string, error) {
	var info *a2s.Info
	err := retryWithContext(ctx, "theisle-a2s-query", 15*time.Second, func(actx context.Context) error {
		var err error
	...
	if err != nil {
		return joindepth.JoinDepth(-1), "", fmt.Errorf("the isle a2s query failed: %w", err)
	}
