// Package main implements a join-depth probe for Team Fortress 2 dedicated servers.
package main
...
func main() {
	...
	depth, evidence, err := probeTF2(ctx, *addr)
...
// probeTF2 executes join-depth probing against the target game server.
func probeTF2(ctx context.Context, addr string) (joindepth.JoinDepth, string, error) {
	var info *a2s.Info
	err := retryWithContext(ctx, "tf2-a2s-query", 15*time.Second, func(actx context.Context) error {
		var err error
	...
	if err != nil {
		return joindepth.JoinDepth(-1), "", fmt.Errorf("tf2 a2s query failed: %w", err)
	}
	...
	_ = retryWithContext(ctx, "tf2-source-challenge", 5*time.Second, func(actx context.Context) error {
