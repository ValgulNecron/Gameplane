// Package main implements a join-depth probe for Hell Let Loose dedicated servers.
package main
...
func main() {
	...
	depth, evidence, err := probeHLL(ctx, *addr)
...
func probeHLL(ctx context.Context, addr string) (joindepth.JoinDepth, string, error) {
	...
	if err != nil {
		return joindepth.JoinDepth(-1), "", fmt.Errorf("hell let loose a2s query failed: %w", err)
	}
