// Package main implements a join-depth probe for Mount & Blade II: Bannerlord dedicated servers.
package main
...
func main() {
	...
	depth, evidence, err := probeBannerlord(ctx, *addr)
...
// probeBannerlord executes join-depth probing against the target game server.
func probeBannerlord(ctx context.Context, addr string) (joindepth.JoinDepth, string, error) {
	var info *a2s.Info
	err := retryWithContext(ctx, "bannerlord-a2s-query", 15*time.Second, func(actx context.Context) error {
		var err error
	...
	if err != nil {
		return joindepth.JoinDepth(-1), "", fmt.Errorf("bannerlord a2s query failed: %w", err)
	}
