// Package main implements a join-depth probe for Arma Reforger dedicated servers.
package main
...
func main() {
	...
	depth, evidence, err := probeArmaReforger(ctx, *addr)
...
// probeArmaReforger executes join-depth probing against the target game server.
func probeArmaReforger(ctx context.Context, addr string) (joindepth.JoinDepth, string, error) {
	var info *a2s.Info
	err := retryWithContext(ctx, "arma-reforger-a2s-query", 15*time.Second, func(actx context.Context) error {
		var err error
	...
	if err != nil {
		return joindepth.JoinDepth(-1), "", fmt.Errorf("arma reforger a2s query failed: %w", err)
	}
