...
// holds the gameplane.local/module-finalizer while any GameServer references
...
func TestModule_FinalizerBlocksWhileTemplateInUse(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		hasOurs := false
		for _, f := range fins {
			if f == "gameplane.local/module-finalizer" {
				hasOurs = true
				break
			}
		}
		if !hasOurs {
			t.Fatalf("module mid-delete missing gameplane.local/module-finalizer (finalizers=%v)", fins)
		}
