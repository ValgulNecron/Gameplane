...
- **Network capture stop flow (F-259; maintainer decision 2026-09-25):** The API never writes a capture's terminal phase. `POST :capture-stop` and `:capture-disable` only set the `gameplane.local/stop-requested` annotation (value: RFC3339 UTC time of the first request; set only if absent, so a repeat stop is a no-op). No CRD schema change. On reconcile, a NetworkCapture carrying the annotation in a non-terminal phase is handled before any other branch (`completeRequestedStop`):
  - **Pending (or not yet reconciled):** `SidecarClient.GetCaptureStatus()` is asked first, because the Pending branch starts the sidecar before its own status write and a stop request landing in between leaves the object Pending while the sidecar captures. If the sidecar knows the capture, it is stopped exactly as in the Running case (reason `stopped`). Only when the status check errors (the sidecar has no record of it) is it set to Completed directly with `message="stopped by user request"` and `SidecarStopped=True` (reason `never_started`), with no stop call.
