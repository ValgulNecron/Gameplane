...
- **Network capture stop flow (F-259; maintainer decision 2026-09-25):** The API never writes a capture's terminal phase. `POST :capture-stop` and `:capture-disable` only set the `gameplane.local/stop-requested` annotation (value: RFC3339 UTC time of the first request; set only if absent, so a repeat stop is a no-op). No CRD schema change. On reconcile, a NetworkCapture carrying the annotation in a non-terminal phase is handled before any other branch (`completeRequestedStop`):
  - **Pending (or not yet reconciled):** it never reached the sidecar, so it is set to Completed directly with `message="stopped by user request"` and `SidecarStopped=True` (reason `never_started`); no sidecar call.
  - **Running:** `SidecarClient.StopCapture()` is called first (the sidecar's `:stop` is synchronous and closes/flushes the PCAPNG), then the GameServer's `status.capture.activeCapture` lock is released and the capture is set to Completed with `message="stopped by user request"` and `SidecarStopped=True` (reason `stopped`). A failed sidecar stop still completes the capture and records `SidecarStopFailed=True`.
  ...
  - **Rolling-upgrade fallback:** a Completed capture with `message="stopped by user request"` and no `SidecarStopped` condition (written by a pre-F-259 API, or by `GameServerReconciler.stopActiveCaptures` on capture disable) still gets exactly one `StopCapture` call, guarded by `SidecarStoppedCondition`.
